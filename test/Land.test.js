...
contract('Land', (accounts) => {
  ...
  describe('create', () => {
    it('creator can create', async () => {
      await this.land.create(0, 1, 1, { from: creator })
      const landExists = await this.land.existsLand(0, 1, 1)
      landExists.should.be.equal(true)
      const landOwner = await this.land.ownerOfLand(0, 1, 1)
      landOwner.should.be.equal(creator)
    })
    it('world owner can create', async () => {
      await this.land.create(0, 1, 2, { from: worldOwner })
      const landExists = await this.land.existsLand(0, 1, 2)
      landExists.should.be.equal(true)
      const landOwner = await this.land.ownerOfLand(0, 1, 2)
      landOwner.should.be.equal(worldOwner)
    })
    it('unauthorized address cannot create', async () => {
      await assertRevert(this.land.create(0, 2, 2, { from: accounts[3] }))
    })
    it('authorized address can create', async () => {
      const recipient = accounts[2]
      await this.land.authorize(recipient, { from: creator })
      await this.land.create(0, 2, 1, { from: accounts[2] })
      const landExists = await this.land.existsLand(0, 2, 1)
      landExists.should.be.equal(true)
      const landOwner = await this.land.ownerOfLand(0, 2, 1)
