...
contract('LandPotAuction', (accounts) => {
  ...
  describe('bid', () => {
    ...
    beforeEach(async () => {
      ({ logs: this.logs } = await this.landPotAuction.bid(1, 1, team1, bid1, { from: bidder1, value: bid1 }))
    })
    ...
    describe('failed bid with less than max bid', () => {
      beforeEach(async () => {
        ({ logs: this.logs } = await this.landPotAuction.bid(1, 1, team2, bid2failed, { from: bidder2, value: bid2failed }))
      })
    ...
    describe('succeed bid with higher than max bid', () => {
      beforeEach(async () => {
        ({ logs: this.logs } = await this.landPotAuction.bid(1, 1, team2, bid2success, { from: bidder2, value: bid2success }))
      })
      it('current bidder and bid changed', async () => {
        const data = await this.landPotAuction.getPlot.call(1, 1)
  ...
  describe('finalizeAuction', () => {
    ...
    beforeEach(async () => {
      ...
      await this.landPotAuction.bid(1, 2, team2, ether(0.5), { from: bidder2, value: ether(0.5) })
      await this.landPotAuction.bid(1, 2, team1, ether(0.7), { from: bidder1, value: ether(0.4) }) // Bidder 1 has 0.3 balance
      await this.landPotAuction.bid(1, 3, team2, ether(0.8), { from: bidder2, value: ether(0.3) }) // Bidder 2 has 0.5 balance
      await this.landPotAuction.bid(1, 3, team1, ether(0.9), { from: bidder3, value: ether(0.9) })
    })
    it('bidder 2 win (1,1)', async () => {
      const data = await this.landPotAuction.getPlot(1, 1)
      data[2].should.eq(bidder2)
      data[3].should.be.bignumber.equal(team2)
      data[4].should.be.bignumber.equal(ether(0.3))
    })
    it('bidder 1 win (1,2)', async () => {
      const data = await this.landPotAuction.getPlot(1, 2)
      data[2].should.eq(bidder1)
      data[3].should.be.bignumber.equal(team1)
      data[4].should.be.bignumber.equal(ether(0.5))
    })
    it('bidder 3 win (1,3)', async () => {
      const data = await this.landPotAuction.getPlot(1, 3)
    ...
    describe('finalize auction', () => {
      beforeEach(async () => {
        await this.landPotAuction.endAuction({ from: creator });
        ({ logs: this.logs } = await this.landPotAuction.finalizeAuction(bidder3, team1, { from: creator }))
      })
      it('bidder1 refunded and rewarded', async () => {
        const jackpot = await this.landPotAuction.jackpot()
        const balance = await this.landPotAuction.balances(bidder1)
      ...
      it('bidder2 refunded', async () => {
        const balance = await this.landPotAuction.balances(bidder2)
        balance.should.be.bignumber.equal(ether(0.9)) // 0.8+(0.4-0.3)
      })
      it('bidder3 refunded and rewarded', async () => {
        const balance = await this.landPotAuction.balances(bidder3)
