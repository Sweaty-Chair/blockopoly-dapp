...
contract LandPotAuction is Pausable {
  ...
  // Variables to keep track of current auction
  Auction public currentAuction;
  mapping(uint32 => Auction[]) public pastAuctions; // World => Auctions
  mapping(uint256 => Auction) public landAuction; // Land (token ID) => Auction
  ...
  function startAuction(int64 x, int64 y) public onlyOwner {
    ...
    for (uint8 k = 0; k < PLOT_AREA; k++) {
      ...
      currentAuction.plots[k] = Plot(i, j, address(0), 0, 0, 0); // Creates new struct and saves in storage
    }
  }
   ...
   */
  function getPlots() external view returns (int8[] xs, int8[] ys, address[] bidders, uint256[] currentBids) {
  ...
  function bid(int8 i, int8 j, uint8 team) external payable whenNotPaused canBid {
    ...
    Plot storage plot = currentAuction.plots[k];
    uint256 thisBid = balances[msg.sender].add(msg.value);
    require(thisBid >= plot.currentBid.add(1 finney)); // Must larger than current bid
    if (thisBid <= plot.maxBid) { // Failed to outbid current bidding, less than its max bid
      emit OutBid(plot.x, plot.y, plot.bidder, msg.sender, team, thisBid);
      thisBid = thisBid.add(1 finney); // Add a finney to the current bid
      plot.currentBid = thisBid; // Increase the current bid
      emit OutBid(plot.x, plot.y, msg.sender, plot.bidder, plot.team, thisBid);
      revert();
    }
  ...
  /**
   * @dev Withdraws the earned ethers while keeping enough for jackpot and outbid balances, owner only.
   */
  ...
  function test1() public view returns (int8) {
    return currentAuction.plots[0].x;
  }
